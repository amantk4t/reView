import { c as create_ssr_component, h as each, v as validate_component, f as add_attribute, e as escape } from "../../../../chunks/index.js";
import { M as Movie } from "../../../../chunks/Movie.js";
const MovieCarousel_svelte_svelte_type_style_lang = "";
const css$1 = {
  code: "#carousel.svelte-4l0gsc.svelte-4l0gsc{overflow-x:auto;overflow-y:hidden;white-space:nowrap;-webkit-overflow-scrolling:touch;-ms-overflow-style:none;scroll-behavior:smooth}#carousel.svelte-4l0gsc:hover #carousel-item.svelte-4l0gsc,#carousel-container.svelte-4l0gsc:hover #carousel #carousel-item.svelte-4l0gsc{animation-play-state:paused}#carousel.svelte-4l0gsc.svelte-4l0gsc::-webkit-scrollbar{display:none}#carousel-item.svelte-4l0gsc.svelte-4l0gsc{scroll-snap-align:start;scroll-snap-stop:always;animation:svelte-4l0gsc-scroll 55s linear infinite}#carousel-container.svelte-4l0gsc button.svelte-4l0gsc{transition:transform 0.2s ease-in-out}#carousel-container.svelte-4l0gsc button.svelte-4l0gsc:hover{transform:scale(1.1) rotate()}@keyframes svelte-4l0gsc-scroll{0%{transform:translateX(0)}100%{transform:translateX(calc(-100% + 100vw))}}",
  map: null
};
const MovieCarousel = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div id="${"carousel-container"}" class="${"relative svelte-4l0gsc"}"><div id="${"carousel"}" class="${"max-w-screen overflow-x-auto  svelte-4l0gsc"}"><section id="${"carousel-item"}" class="${"flex min-w-screen w-fit gap-4 items-center justify-around svelte-4l0gsc"}">${each(similar, (movie) => {
    return `${validate_component(Movie, "Movie").$$render($$result, { movie, carousel: true }, {}, {})}`;
  })}</section></div>
	<button class="${"bg-gray-900/60 rounded-full p-2 absolute left-3 top-1/3 svelte-4l0gsc"}"><svg class="${"w-10 text-gray-100"}" fill="${"currentColor"}" viewBox="${"0 0 24 24"}" aria-hidden="${"true"}" role="${"presentation"}"><path d="${"M7.47,11.152l8-5C15.779,5.958,16.167,5.949,16.485,6.125C16.803,6.302,17,6.636,17,7v10 c0,0.364-0.197,0.698-0.515,0.875C16.333,17.958,16.167,18,16,18c-0.184,0-0.368-0.051-0.53-0.152l-8-5C7.178,12.665,7,12.345,7,12 S7.178,11.335,7.47,11.152z"}"></path></svg></button>
	<button class="${"rotate-180 bg-gray-900/60 rounded-full p-2 absolute right-3 top-1/3 svelte-4l0gsc"}">
...
const YouTubePlayer = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  video = {};
  loading = false;
  return `<div class="${"relative py-6 md:mb-0 w-[75vw] mx-auto h-[55vh] min-h-[400px] max-w-full"}">${video.title ? `<div class="${"absolute w-full h-[55vh] top-6 left-0 z-30 rounded-lg"}"><div class="${"relative w-full h-full"}"><iframe${add_attribute("title", video.title, 0)}${add_attribute("src", `https://www.youtube.com/embed/${video.videoId}?autoplay=1`, 0)} frameborder="${"0"}" allowfullscreen class="${"absolute top-0 left-0 w-full h-full rounded-lg"}"></iframe></div></div>` : `<img${add_attribute("alt", movie.title, 0)}${add_attribute("src", `https://image.tmdb.org/t/p/original${movie.poster_path}`, 0)} class="${"absolute z-40 object-cover w-full h-[55vh] shadow-lg rounded-lg"}">
		<button aria-label="${"Play Video"}" class="${[
    "absolute inset-0 top-6 flex items-center justify-center w-screen max-w-full h-[55vh] transition-colors duration-300 bg-gray-800 bg-opacity-50 group hover:bg-opacity-25 rounded-lg z-40",
    loading ? "loading" : ""
...
<section class="${"dark:bg-gray-900"}"><div class="${"flex justify-center"}"><a href="${"/"}" class="${"flex items-center justify-center w-10 h-10 transition duration-300 transform dark:bg-gray-200 rounded-full shadow-2xl absolute top-5 left-5 z-50 hover:bg-gray-100"}"><svg xmlns="${"http://www.w3.org/2000/svg"}" class="${"w-5 h-5 text-gray-800 dark:text-gray-800"}" fill="${"none"}" viewBox="${"0 0 24 24"}" stroke="${"currentColor"}"><path stroke-linecap="${"round"}" stroke-linejoin="${"round"}" stroke-width="${"4"}" d="${"M10 19l-7-7m0 0l7-7m-7 7h18"}"></path></svg></a></div>
	<div class="${"max-w-full"}">${validate_component(YouTubePlayer, "YouTubePlayer").$$render($$result, { movie }, {}, {})}
		<div class="${"flex flex-col items-center justify-center text-center"}">${movie.title !== void 0 && !loading ? `<h2 class="${"mb-5 font-sans text-3xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-5xl sm:leading-none"}">${escape(movie.title)}</h2>
				<p class="${"mb-5 text-base text-gray-700 dark:text-gray-400 max-w-md break-words"}">${escape(movie.overview)}</p>
				<div class="${"flex items-center md:justify-center"}"><a href="${"/"}" class="${"inline-flex items-center justify-center h-12 px-6 mr-6 font-medium tracking-wide text-white transition duration-200 rounded shadow-md bg-orange-400 hover:bg-orange-500 focus:shadow-outline focus:outline-none"}"><svg class="${"w-10 text-gray-100"}" fill="${"currentColor"}" viewBox="${"0 0 24 24"}"><path d="${"M16.53,11.152l-8-5C8.221,5.958,7.833,5.949,7.515,6.125C7.197,6.302,7,6.636,7,7v10 c0,0.364,0.197,0.698,0.515,0.875C7.667,17.958,7.833,18,8,18c0.184,0,0.368-0.051,0.53-0.152l8-5C16.822,12.665,17,12.345,17,12 S16.822,11.335,16.53,11.152z"}"></path></svg>
						Watch now
